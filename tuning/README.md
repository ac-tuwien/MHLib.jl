...
`julia-server.jl` is a simple standalone server program, which first needs to be started, 
...
Now, the script `target-runner` from `irace` sends the function calls to the server 
via the socket. 
This way, the overhead of starting a new Julia process for each function call is avoided.
When using this variant of applying irace, ensure to adapt also `target-runner`
concerning your parameters.
Note that the server evaluates any Julia expression it receives; it listens on
`localhost` only and is meant solely for local tuning runs.
